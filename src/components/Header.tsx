...
const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-4">
          <nav className="flex justify-between items-center">
            <Link to="/" className="flex items-center transition-smooth hover-scale">
            ...
            <div className="hidden md:flex items-center space-x-8">
              <Link 
                to="/" 
                className={`${isScrolled ? 'text-gray-800' : 'text-white'} hover:text-red-500 transition-all duration-300 whitespace-nowrap text-xs relative group`}
              >
                Strona główna
            ...
            >
              {isOpen ? (
                <X className={`${isScrolled ? 'text-gray-800' : 'text-white'} transition-all duration-300`} size={24} />
              ) : (
                <Menu className={`${isScrolled ? 'text-gray-800' : 'text-white'} transition-all duration-300`} size={24} />
              )}
            </button>
          </nav>
        </div>
      ...
      <div className={`h-32 ${location.pathname === '/' ? 'hidden' : 'block'}`}></div>
