...
const SpeakersSection: React.FC = () => {
  ...
  const parseDescription = (speaker) => {
    if(speaker.link){
      ...
      return(<>{before}<a href={speaker.link.url} target='_blank' className="mt-2 text-red-500">{speaker.link.text}</a>{after}</>)
    }
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16 animate-fade-in">
          ...
          <p className="text-base max-w-4xl mx-auto leading-relaxed text-gray-700 transition-all duration-500">
            Poznaj ekspertów, którzy dzielą się swoją wiedzą i doświadczeniem w ramach kampanii.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {speakers.map((speaker, index) => (
            ...
            >
              <div className="relative overflow-hidden bg-gray-100 flex items-center justify-center transition-all duration-500" style={{ minHeight: '200px' }}>
                <img 
                  src={speaker.photo} 
                  alt={speaker.name} 
              ...
              <div className="p-6 flex-grow">
                <h3 className="text-base font-bold mb-2 transition-all duration-300">{speaker.name}</h3>
                <p className="text-xs text-red-500 font-medium mb-3 transition-all duration-300">{speaker.title}</p>
                {speaker.description && (
                  <div className="relative">
                    <div className={`text-gray-600 overflow-hidden transition-all duration-500 ${
                      expandedSpeakers.includes(speaker.id) ? 'max-h-[1000px]' : 'max-h-24'
                    }`}>
                      <p className={`text-xs ${expandedSpeakers.includes(speaker.id) ? '' : 'line-clamp-3'}`}>
                    ...
                    {speaker.description && (
                      ...
                      >
                        {expandedSpeakers.includes(speaker.id) ? (
                          <>
                            Zwiń
                            <ChevronUp size={14} className="ml-1 transition-transform duration-300" />
                          </>
                        ) : (
                          <>
                            Rozwiń
                            <ChevronDown size={14} className="ml-1 transition-transform duration-300" />
                          </>
                        )}
                      </button>
                    )}
